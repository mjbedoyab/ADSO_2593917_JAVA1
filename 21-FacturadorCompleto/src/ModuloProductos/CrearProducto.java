...
public class CrearProducto extends javax.swing.JFrame {
    ...
    public CrearProducto(Menu ventanaMenu, Producto [] listaProductos, String titulo) {
    ...
    if (!codigo.equals("") && !nombre.equals("") && !precio.equals("")) {
        boolean repetido = (this.titulo.equalsIgnoreCase("PRODUCTOS")) ? (this.ventanaMenu.database.buscarProducto(codigo) != null) : false;
        

        if (!repetido) {
            boolean proceso = this.ventanaMenu.database.insertarProducto(codigo, nombre, precio);
            ...
            if (proceso) {
                ...
                Alert alerta = new Alert("ERROR", "Ocurrió un error al insertar el producto.", "error");
            }
        } else {
            Alert alerta = new Alert("CODIGO", "El código ingresado se encuentra repetido.", "error");
        }
    } else {
